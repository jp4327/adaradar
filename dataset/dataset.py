...
class RADIal(Dataset):
    ...
    def __init__(self, root_dir,statistics=None,encoder=None,difficult=False):
        ...
        self.labels = pd.read_csv('PLEASE_REPLACE_WITH_YOUR_DIR_TO_labels.csv').to_numpy()
       
        # map: sample_id  ➜  sequence string ("RECORD@...")
        self.sample_to_seq = {row[0]: row[14] for row in self.labels}
    ...
    def __getitem__(self, index):
        ...
        seq_id   = self.sample_to_seq[sample_id]     # look‑up
        ...
        return radar_FFT, segmap,out_label,box_labels,image, seq_id