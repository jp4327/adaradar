...
def GetFullMetrics(predictions,object_labels,range_min=5,range_max=100,IOU_threshold=0.5):
    ...
    # JH Mod
    return perfs, RangeError, AngleError

def GetFullMetrics_withPerImage(predictions, object_labels, range_min=5, range_max=100, IOU_threshold=0.5):
    perfs = {}
    precision = []
    recall = []
    iou_thresholds = []
    ...
    # NEW: Store per-image precision results in a dict:
    # - Key: threshold value
    # - Value: list of per-image precision for each frame at this threshold
    per_image_precision_dict = {}
    per_image_recall_dict = {}

    for threshold in np.arange(0.1, 0.96, 0.1):
        
        iou_thresholds.append(threshold)
        
        # Global accumulators (as in your original code)
        TP_global = 0
        FP_global = 0
        FN_global = 0
        
        range_error_global = 0
        angle_error_global = 0
        nbObjects_global = 0
        
        # NEW: keep track of per-image precision
        per_image_precision_list = []
        per_image_recall_list = []
        ...
        for frame_id in range(len(predictions)):
            ...
            # If both predictions and GT exist for this frame
            if len(ground_truth_box_corners) > 0 and len(Object_predictions) > 0:
                used_gt = np.zeros(len(ground_truth_box_corners))
                ...
                for pid, prediction in enumerate(Object_predictions):
                    ...
                    if len(ids) > 0:
                        TP_local += 1
                        used_gt[ids] = 1
                        # accumulate errors for these matches
                        range_error_global += np.sum(np.abs(ground_truth_box_corners[ids, -2] - prediction[-2]))
                        angle_error_global += np.sum(np.abs(ground_truth_box_corners[ids, -1] - prediction[-1]))
                        nbObjects_global += len(ids)
                    else:
                        FP_local += 1

                FN_local = np.sum(used_gt == 0)
            
            elif len(ground_truth_box_corners) == 0:
                # No ground truths, all predictions are false positives
                FP_local = len(Object_predictions)

            elif len(Object_predictions) == 0:
                # No predictions, everything in ground truths is missed
                FN_local = len(ground_truth_box_corners)
            ...
            # Compute per-image precision for this frame
            if (TP_local + FP_local) > 0:
                frame_precision = TP_local / (TP_local + FP_local)
                frame_recall = TP_local / (TP_local + FN_local)

            else:
                frame_precision = 0.0
                frame_recall = 0.0

            # Add to the list for the current threshold
            per_image_precision_list.append(frame_precision)
            per_image_recall_list.append(frame_recall)

        # Done looping over frames
        
        # Add to dictionary
        per_image_precision_dict[threshold] = per_image_precision_list
        per_image_recall_dict[threshold] = per_image_recall_list

        # Now compute dataset-level precision/recall for this threshold
        if TP_global != 0:
            precision_val = TP_global / (TP_global + FP_global)
            recall_val = TP_global / (TP_global + FN_global)
        else:
            precision_val = 0.0
            recall_val = 0.0
        ...
        if nbObjects_global > 0:
            RangeError.append(range_error_global / nbObjects_global)
            AngleError.append(angle_error_global / nbObjects_global)
        else:
            RangeError.append(0.0)
            AngleError.append(0.0)

    # Summarize in `perfs`
    perfs['precision'] = precision
    perfs['recall'] = recall
    
    # F1 score using mean(precision) and mean(recall)
    F1_score = (np.mean(precision) * np.mean(recall)) / ((np.mean(precision) + np.mean(recall)) / 2 + 1e-9)
    ...
    # Return everything plus the new dictionary of per-image precision
    return perfs, RangeError, AngleError, per_image_precision_dict, per_image_recall_dict
